...
vi.mock("../components/OnlineGameBrowser", () => ({
  default: ({
    ...
    account?: { identity: { kind: string; id: string; displayName?: string }; displayName: string };
    resolveAccountGameJoin?: (
      game: {
        gameId: string;
      ...
      seat: "w" | "b"
    ) => { gameId: string; seat: "w" | "b"; token: string } | null;
    onReturnToAccountGame?: (
    ...
    <div>
      ...
      {account && resolveAccountGameJoin && onReturnToAccountGame && (
        <button
          type="button"
          onClick={() => {
            const activePrivateAccountGame = {
              schemaVersion: ONLINE_GAME_SUMMARY_SCHEMA_VERSION,
              gameId: "game_private_account_return",
              ...
              lastEventId: "evt_private_account_return",
            };
            const join = resolveAccountGameJoin(activePrivateAccountGame, "w");
            if (join) onReturnToAccountGame(join, activePrivateAccountGame.visibility);
          }}
        >
...
describe("App game setup lifecycle", () => {
  ...
  it("returns to a private active account game without treating it as unlisted", async () => {
    const account = {
      schemaVersion: 1 as const,
      accountId: "account_private_return",
      displayName: "Liam",
      createdAt: "2026-06-03T12:00:00.000Z",
      updatedAt: "2026-06-03T12:00:00.000Z",
      identity: { kind: "registered" as const, id: "account_private_return", displayName: "Liam" },
    };
    rememberOnlineAccountSession({
    ...
    render(<App />);
    fireEvent.click(screen.getByRole("button", { name: "Open Online" }));
    fireEvent.click(await screen.findByRole("button", { name: "Mock Return Active Account Game" }));

    await waitFor(() => {
      expect(onlineHookMocks.useOnlineGameConnection).toHaveBeenLastCalledWith(
        { gameId: "game_private_account_return", seat: "w", token: "white-seat-token" },
        expect.any(Function)
      );
    });
    expect(window.location.search).toContain("onlineGame=game_private_account_return");
    expect(window.location.search).toContain("seat=w");
    expect(window.location.search).not.toContain("token=");

    const playerCallback = onlineHookMocks.useOnlineGameConnection.mock.calls.at(-1)?.[1];
    act(() => {
      playerCallback(spectatorSnapshot("game_private_account_return"));
    });

    await waitFor(() => {
      expect(screen.getByText("Online session: player")).toBeInTheDocument();
      expect(screen.getByText("Online visibility: private")).toBeInTheDocument();
      expect(screen.getByText("Online spectator URL: none")).toBeInTheDocument();
    });
  ...
  it("deletes the saved online account only after the server confirms deletion", async () => {
    const account = {
      schemaVersion: 1 as const,
      ...
      createdAt: "2026-06-03T12:00:00.000Z",
      updatedAt: "2026-06-03T12:00:00.000Z",
      identity: { kind: "registered" as const, id: "account_delete", displayName: "Liam" },
    };
    rememberOnlineAccountSession({
      sessionId: "account-session",
      token: "account-token",
    ...
    pendingDelete.resolve(
      new Response(
        JSON.stringify({ protocolVersion: 1, deleted: true }),
        { status: 200, headers: { "content-type": "application/json" } }
      )
    );
  ...
  it("keeps the account session available when account deletion fails", async () => {
    const account = {
      schemaVersion: 1 as const,
      accountId: "account_delete_retry",
      displayName: "Liam",
      createdAt: "2026-06-03T12:00:00.000Z",
      updatedAt: "2026-06-03T12:00:00.000Z",
      identity: { kind: "registered" as const, id: "account_delete_retry", displayName: "Liam" },
    };
    rememberOnlineAccountSession({
      sessionId: "account-session",
      token: "account-token",
  ...
  it("offers signed-in report and block actions from registered direct challenge links", async () => {
    const account = {
      schemaVersion: 1 as const,
    ...
    const fetchMock = vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (path === "/api/online/account/reports/Samir") {
    ...
    fireEvent.change(within(reportForm).getByRole("textbox", { name: "Details" }), {
      target: { value: "Direct challenge spam." },
    });
    fireEvent.click(within(reportForm).getByRole("button", { name: "Submit Report" }));

    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith(
      "/api/online/account/reports/Samir",
      expect.objectContaining({ method: "POST" })
    ));
    expect(await within(accountActions).findByText("Report submitted for Samir.")).toBeInTheDocument();

    fireEvent.click(within(accountActions).getByRole("button", { name: "Block Samir from challenge link" }));

    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith(
      "/api/online/account/blocks/Samir",
        ...
        return Promise.resolve(
          new Response(
            JSON.stringify({
              ...
              challengeId: "challenge_samir",
              challenger: {
                url: "https://castles.example/?onlineChallenge=challenge_samir&challengeRole=challenger#challengeToken=liam-secret",
              },
              challenged: {
                url: challengedUrl,
              },
              summary: {
                ...
                challengerSeat: "w",
                visibility: "unlisted",
                setup: { board: { config: { nSquares: 8 }, castles: [] }, pieces: [], sanctuaries: [] },
                createdAt: "2026-06-01T12:00:00.000Z",
                updatedAt: "2026-06-01T12:00:00.000Z",
                expiresAt: "2026-06-02T12:00:00.000Z",
                status: "pending",
                lastEventId: "challenge_samir_evt_created",
              },
            }),
            { status: 201, headers: { "content-type": "application/json" } }
          )
