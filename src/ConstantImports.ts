...
const size_hexes =
  ...
  (HEX_SIZE_FACTORc * NSquaresc);
const hexSize = new Point(size_hexes, size_hexes);
const origin = new Point(
  window.innerWidth / 2 + X_OFFSETc,
  window.innerHeight / 2
);
export const layout = new Layout(
  layoutTypec === "flat" ? Layout.flat : Layout.pointy,
  hexSize,
  origin
);
...
export const riverHexes = startingBoard.hexes.filter((hex: Hex) =>
  isRiver(layout.hexToPixel(hex), origin)
);
export const castleHexes = startingBoard.hexes.filter((hex: Hex) =>
  isCastle(hex, NSquaresc - 1)
);
export const whiteCastleHexes = castleHexes.filter((hex: Hex) =>
  isWhiteCastle(hex, NSquaresc - 1)
);
