...
Default production remains `CASTLES_DEPLOYMENT_MODE=single-node` or unset. `CASTLES_DEPLOYMENT_MODE=multi-instance` is now a guarded private-beta mode for a full PostgreSQL runtime stack: runtime-node state, live spectator presence, runtime-event fanout hints, selected operation gates, shared rate limits, startup maintenance ownership, warm room hydration from the store, event polling readiness, heartbeat readiness, and rolling-drain behavior must all be configured and healthy. Do not treat this as a public capacity or autoscaling claim; before adding nginx load balancing, PM2 cluster mode, Node cluster workers, or multiple systemd app instances, run the two-node PostgreSQL rehearsal and decide the production routing/deploy shape explicitly.
...
If localhost health works but public health fails, the remaining problem is nginx, SSL, DNS, or firewall routing rather than the Node app. If `server:check-config` fails, fix the PostgreSQL URL, permissions, schema-readiness error, or deployment-mode guard before restarting. `server:check-config` reports `onlineDeployment` and `/api/health` reports `online.deployment`; single-node reports `multiInstanceReady: false`, while guarded multi-instance reports `multiInstanceReady: true` only with PostgreSQL runtime-event fanout, live spectator presence, store-authoritative warm room state, PostgreSQL queue guards, and multi-node routing. Do not reinterpret either mode as a public capacity or autoscaling signal. If health works but the browser shows old UI, verify the health commit, hard-refresh once, and rerun the browser smoke; the current app is designed to bypass stale app-shell caching after deploys.
