...
The current online service is single-node only. Keep `CASTLES_DEPLOYMENT_MODE=single-node` or leave it unset. The production runtime coordinator now uses PostgreSQL for live spectator presence, selected operation gates, shared rate limits, runtime event outbox records, and startup maintenance ownership, but the server still deliberately refuses `CASTLES_DEPLOYMENT_MODE=multi-instance`. Do not run nginx load balancing, PM2 cluster mode, Node cluster workers, or multiple systemd app instances for the same database, even with sticky routing. PostgreSQL runtime stores do not yet provide cross-instance WebSocket fanout, a production runtime-event polling scheduler, warm room cache invalidation/readiness across workers, complete queue/gate coverage, or two-instance validation. Before running multiple Node workers or app instances, add those missing runtime pieces, rolling-drain behavior for WebSockets, and multi-instance tests.
...
- `Alert: deployment_not_single_node severity=critical`: production health is missing the supported single-node guardrail or is advertising unsupported multi-instance readiness. Check `CASTLES_DEPLOYMENT_MODE`, ensure only one Node app instance is serving the database, and do not enable load-balanced app workers until WebSocket fanout, runtime-event polling readiness, cache invalidation, full queue/gate coverage, rolling drain, and two-instance tests exist.
