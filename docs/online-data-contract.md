...
PostgreSQL locks and runtime stores protect accepted actions, timeouts, selected seek/challenge mutations, rate limits, live spectator presence, runtime event polling, and startup maintenance ownership, but they do not fan out WebSocket snapshots, invalidate another process's warm room cache by themselves, or prove every process-local queue/gate is global. A deployment may claim multi-instance readiness only after those separate runtime concerns are implemented and tested.
