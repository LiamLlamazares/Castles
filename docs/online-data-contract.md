...
`livePreview.spectatorCount` is response-only presence metadata. It is allowed only on active summaries, is omitted when the current count is zero or unknown, and must not be stored as durable archive/history data. In the current PostgreSQL production runtime it is backed by PostgreSQL live spectator presence. A supported single-node deployment reports `online.deployment.multiInstanceReady: false`, process-local WebSocket fanout, process-local room state, process-local queue guards, and single-node routing. A supported multi-instance deployment may report `multiInstanceReady: true` only with PostgreSQL runtime-event WebSocket fanout hints, PostgreSQL live spectator presence, store-authoritative warm room hydration, PostgreSQL lock/transaction queue guards, session-store account presence, and multi-node routing.

`livePreview.spectatorCount` must also stay independent from durable account presence. Account public-profile presence is derived from account session activity and privacy policy; game spectator count is derived from the live spectator presence runtime. Clients must not merge them or use spectator counts for public capacity claims or archival statistics.

PostgreSQL locks and runtime stores protect accepted actions, timeouts, selected seek/challenge mutations, rate limits, live spectator presence, runtime event polling, runtime-node drain/heartbeat state, and startup maintenance ownership. A deployment may claim multi-instance readiness only through `CASTLES_DEPLOYMENT_MODE=multi-instance` with the complete PostgreSQL runtime stack configured and the health metadata above. That readiness is an operational runtime contract, not a public capacity or autoscaling claim.
...
Directory and summary response shapes are strict at the browser boundary. Unsupported fields in the directory envelope, summary, participants, result, live preview, last move, clock, board preview, board-preview pieces, or board-preview castles are malformed and rejected before the browser uses the response. This applies to public directory summaries and to authenticated account history/head-to-head summaries because they share the same `OnlineGameDirectoryResponse` and `OnlineGameSummary` validators.
...
The optional-account action routes `POST /api/online/games`, `POST /api/online/seeks`, `POST /api/online/seeks/:seekId/accept`, `POST /api/online/matchmaking/quick`, and `POST /api/online/challenges` are body-only path routes. They reject query strings for anonymous and account-bearing callers instead of accepting leaked account-session tokens or unsupported client-supplied options.

Identity `id` values in persisted summaries are never authentication secrets. Do not put cookies, bearer tokens, raw private invite tokens, or server auth session ids in `OnlineIdentity.id`. Use a separate private credential table for authentication material. Account-facing challenge and account-history HTTP responses redact registered identity ids to display-name-derived public surrogates so those responses can still show registered display names without exposing raw account ids.

The PostgreSQL store has a personal-history query that can find public, unlisted, and private summaries for a server-resolved identity. `GET /api/online/account/games` exposes that query only after resolving the account bearer token server-side. It accepts `state`, `limit`, `cursor`, and the safe archive filters `clock=timed|casual`, `rating=casual|rated`, `result=white|black|monarch_captured|castle_control|victory_points|resignation|timeout`, and `q=<visible search text>`; those filters apply before cursor/limit pagination. It must not accept a request-body identity, query identity, browser session id, token-like query parameter, or arbitrary visibility override. Its response must not include unsupported fields such as raw account ids, token hashes, bearer tokens, game seat tokens, challenge tokens, open-seek creator tokens, credential metadata, or internal database keys.
...
- Current-account lookup, account-session listing, current-session revocation, sign-out-everywhere revocation, and account deletion are path-only bearer actions. Once an account bearer resolves, these routes reject query strings instead of accepting leaked account-session tokens or unsupported client-supplied options.
- `GET /api/online/account/games?state=active|archived|all&limit=...&cursor=...&clock=timed|casual&rating=casual|rated&result=...&q=...`: requires the account bearer token and returns strict `OnlineGameDirectoryResponse` rows whose participants include the server-authorized account as a redacted registered identity. It may include private and unlisted rows for that account. Clock, rating, result, and search filters use the same validated values as the public archive, but they never accept a client-supplied identity or visibility override, and response validation rejects unsupported fields.
- `GET /api/online/account/games/head-to-head/:displayName?limit=...&cursor=...`: requires the account bearer token and returns completed archived strict `OnlineGameDirectoryResponse` rows whose participants include the server-authorized account and a different-seat registered opponent with the exact normalized display name, both with redacted registered identity ids. It filters before pagination in the personal-history query, does not look up or expose the target profile, and returns an empty list when the authenticated viewer has no authorized pair history for that name.
- `GET /api/online/account/games/:gameId/snapshot`: requires the account bearer token, loads a server-authorized participant snapshot for account rematch/history shortcuts, and does not mint or return game-seat credentials.
...
The account game snapshot and rejoin routes are path-only actions. They reject query strings, including token/auth-looking query parameters, instead of silently accepting leaked credentials or unsupported client-supplied options.
...
- `GET /api/online/admin/reports?status=open|resolved|dismissed&reason=abuse|cheating|spam|impersonation|other&reporter=<displayName>&target=<displayName>&cursor=<opaque>&limit=1..100`: returns the newest moderation reports for operators only when `CASTLES_ADMIN_BEARER_TOKEN` is configured and the request has `Authorization: Bearer <admin-token>`. Missing, disabled, or wrong admin bearer access returns the same hidden `not_found` shape. The route is no-store, defaults `status` to `open`, defaults `limit` to 50, optionally filters by one report reason and exact normalized reporter/target display-name snapshots, then applies opaque cursor pagination and limiting. It returns `{ protocolVersion, schemaVersion: 2, reports, nextCursor? }`, and exposes report id, reporter/target display-name snapshots, reason, details, status, latest moderator note, created/updated timestamps, and nullable `reviewedAt`. Admin report rows are strict response shapes; unsupported fields fail closed before the server serializes the response. They must not expose raw account ids, session ids, bearer tokens, token hashes, database keys, challenge tokens, open-seek tokens, or game credentials.
- `PATCH /api/online/admin/reports/:reportId`: updates a report status for operators only behind the same admin bearer gate. This route is path-only; once the admin bearer resolves, it rejects query strings instead of accepting leaked admin tokens or unsupported client-supplied options. The body accepts `{ status, note? }`, where `status` is `open`, `resolved`, or `dismissed` and `note` is normalized text up to 1000 characters. Notes must not contain token/auth/session/credential-looking material. Missing reports return `not_found`; setting the same status returns `not_allowed` without appending a duplicate audit entry. Successful transitions return `{ protocolVersion, schemaVersion: 2, report, audit }`, where `audit` includes audit id, report id, `status_changed`, actor `admin`, previous/next status, normalized note, and timestamp. Report and audit rows are strict response shapes; unsupported fields fail closed before the server serializes the response. The audit payload must not expose raw account ids, session ids, bearer tokens, token hashes, challenge tokens, open-seek tokens, game credentials, or admin bearer material.
...
Account challenge lists support `state=pending` and `state=all`. Pending lists are the default actionable inbox view; pending rows show their remaining expiry time, and rows inside the short expiry window are marked as expiring soon. All lists include accepted, declined, cancelled, and expired terminal summaries for status history. Targeted challenge creation, direct challenge view/action responses, account challenge lists, and account-token accept, decline, and cancel responses redact registered identity ids before returning summaries, while internal challenge events and authorization checks keep canonical identities. Account challenge directory items and challenge summaries are strict response shapes: unsupported fields such as raw account ids, database keys, credential metadata, token hashes, bearer tokens, game seat tokens, challenge tokens, or open-seek creator tokens are rejected before the browser uses the response. Terminal challenge rows are display-only in the visible UI and must not expose accept, decline, or cancel actions. Account-token accept, decline, and cancel routes are path-only actions; once an account bearer resolves, these routes reject query strings instead of accepting leaked account-session tokens or unsupported client-supplied options.
