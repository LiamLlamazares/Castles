...
`livePreview.spectatorCount` is response-only presence metadata. It is allowed only on active summaries, is omitted when the current count is zero or unknown, and must not be stored as durable archive/history data. In the current single-node deployment it is calculated from connected spectator WebSockets on the serving Node process; it is not a global viewer count. The runtime reports `online.deployment.multiInstanceReady: false` in `/api/health`, with `spectatorPresence: "process-local"` and `accountPresence: "session-store"`, and `CASTLES_DEPLOYMENT_MODE=multi-instance` is rejected until shared presence/pub-sub or an equivalent fanout design exists.

`livePreview.spectatorCount` must also stay independent from durable account presence. Account public-profile presence is derived from account session activity and privacy policy; game spectator count is derived from current process-local spectator sockets. A future shared presence backend may power both surfaces, but until then clients must not merge them or use spectator counts for cross-instance ranking, public capacity claims, or archival statistics.

PostgreSQL locks protect accepted actions, timeouts, and seek/challenge mutations from stale writes, but they do not fan out WebSocket snapshots, invalidate another process's warm room cache, or make process-local queues global. A deployment may claim multi-instance readiness only after those separate runtime concerns are implemented and tested.
