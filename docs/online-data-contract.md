...
`livePreview.spectatorCount` is response-only presence metadata. It is allowed only on active summaries, is omitted when the current count is zero or unknown, and must not be stored as durable archive/history data. In the current single-node production runtime it is backed by PostgreSQL live spectator presence, while WebSocket snapshot fanout remains process-local. The runtime reports `online.deployment.multiInstanceReady: false` in `/api/health`, with `spectatorPresence: "postgres-live-presence"` and `accountPresence: "session-store"`, and `CASTLES_DEPLOYMENT_MODE=multi-instance` is rejected until fanout, warm-cache invalidation, runtime event polling readiness, complete queue/gate coverage, and two-instance validation exist.
...
PostgreSQL locks and runtime stores protect accepted actions, timeouts, selected seek/challenge mutations, rate limits, live spectator presence, and startup maintenance ownership, but they do not fan out WebSocket snapshots, run a production polling scheduler, invalidate another process's warm room cache, or prove every process-local queue/gate is global. A deployment may claim multi-instance readiness only after those separate runtime concerns are implemented and tested.
