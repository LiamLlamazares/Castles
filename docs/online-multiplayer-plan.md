...
2. Account/social privacy and moderation boundary pass.
   ...
   - Current item 2 notes: account challenge and account-history raw account id redaction is implemented with local and production verification passed. Client trusted-error promotion now refuses server messages containing raw online identifiers such as account/session/challenge/game/seek/report ids, with local and production verification passed. Account challenge directory/summary, account-history/head-to-head/shared public game-directory, social/profile/following/privacy, and admin moderation report/audit response shapes now fail closed on unsupported fields with local and production verification passed. The current rating-leaderboard sub-slice makes server responses fail closed when store-returned leaderboard entries or nested public rating summaries contain unsupported internals such as raw account ids, rating engine ids, token hashes, deviation, or volatility; the focused red/green HTTP regression, full HTTP server tests, full PostgreSQL account-store tests, full online client tests, full App tests, full OnlineGameBrowser tests, `server:build`, `git diff --check`, app build, and local code/privacy review pass locally, with production deploy and evidence still outstanding. Continue the privacy pass with remaining response-shape checks before marking item 2 done.
