...
5. UI/navigation/tutorial/save QA.
   ...
   - Closure audit status (2026-06-15): item 5's targeted UI/navigation/tutorial/save QA evidence is complete in the current audit harness: `artifacts/ui-audit/phase6ai-local-layout/metrics.json` contains 72 scenario-viewport results across 24 scenarios with zero violations, including first-run, tutorial overview/lesson/progress-return, save dialog, drawer, Lobby row, player Lobby return, missing-invite access denied, and long online status recovery states at desktop/mobile/short-mobile viewports. Concrete remaining gate before item 6: the live dirty worktree currently has concurrent UI modernization edits outside this slice, and broad `npx vitest run` fails in `src/__tests__/FaviconAssets.test.ts` because the CSS-derived app-board background changed to `#161512` while generated favicon/app-icon assets still contain the previous background value. Do not start item 6 in this workspace until that external asset drift is resolved by the UI modernization owner or explicitly folded into a separate verified slice.
