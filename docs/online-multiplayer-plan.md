...
2. Account/social privacy and moderation boundary pass.
   ...
   - Current item 2 notes: account challenge and account-history raw account id redaction is implemented with local and production verification passed. Client trusted-error promotion now refuses server messages containing raw online identifiers such as account/session/challenge/game/seek/report ids, with local and production verification passed. Account challenge directory and summary response shapes now fail closed on unsupported fields with local verification passed, pending production deploy evidence. Continue the privacy pass with remaining response-shape checks before marking item 2 done.
