...
4. Lobby, Watch, Archive, and account-history scanability.
   ...
   - Remaining item 4 queue after the active slice: friend-aware Archive/account-history filters where they directly improve finding playable opponents, richer authorized head-to-head/history links, and archive detail/search read models only if the current `OnlineGameSummary` payload stops being enough for replay/detail pages.
