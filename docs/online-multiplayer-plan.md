...
- Pick the first non-Done slice from `Next Ordered Slices`; the next planned slice remains item 3, starting with optional source-setup equivalence hardening if source setup loading is available, otherwise broader notification-loop handling.
