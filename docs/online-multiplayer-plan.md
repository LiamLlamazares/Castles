...
3. Challenge inbox and rematch reliability.
   ...
   - Current item 3 notes: account challenge inbox rows now expose report/block actions for visible registered opponents only, backed by existing sanitized handlers and local regression coverage; blocking from a row prunes matching visible challenge rows after the server confirms the block. Dedicated rematch request records, direct invite-link report/block affordances, and broader notification-loop handling remain future work.
