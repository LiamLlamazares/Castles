...
- Deploy: deploy only after rated writes remain restricted to completed rated games with two distinct registered accounts, and the public UI still avoids presenting unfinished leaderboard/profile standings.
...
- Additional friend-facing candidates worth keeping in the backlog: friend challenge presets, last-opponent rematch, friend activity limited to public games, private spectator links for trusted friends, head-to-head history from game/profile rows, and friend-scoped leaderboards once rated results are written. These should be added only when each has clear privacy rules and tests.
...
- Basic account profiles, follows, friend links, and public rating summary fields now exist. Add richer rated profile fields and leaderboards only after rated-game writes and public-read privacy rules are stable. Start with rating/confidence leaderboards; treat games-played and LOS-style/Bayesian metrics as optional analytics that need a separate product and math review.
