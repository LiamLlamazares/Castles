...
- Fold in the most useful Lichess-like friend shortcuts once the base graph is stable: pin favorite friends, follow a recent opponent from the game end screen, challenge or rematch a followed player with the current setup, see followed players' public live games, jump to a friend-vs-friend head-to-head history, and show lightweight challenge notifications with accept/decline/cancel states.
- Additional friend-facing candidates worth keeping in the backlog: an "online now" friend rail, friend challenge presets, last-opponent rematch, friend activity limited to public games, private spectator links for trusted friends, head-to-head history from game/profile rows, and friend-scoped leaderboards once rated results are written. These should be added only when each has clear privacy rules and tests.
