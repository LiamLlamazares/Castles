...
Status: first backend/client-helper slice implemented locally on 2026-06-04. Social v1 now has authenticated exact profile lookup, one-way follow/unfollow, following-list reads, block/unblock, privacy get/patch, PostgreSQL persistence, memory-store parity, and strict client response validation that rejects internal identity fields in profile payloads. This slice intentionally does not add visible UI, online/last-seen presence, friend-only lobby listings, friend Watch/Archive filters, challenge inboxes, private notes, challenge presets, ratings, or leaderboards yet.
