...
2. Account/social privacy and moderation boundary pass.
   ...
   - Current item 2 notes: account challenge and account-history raw account id redaction is implemented with local and production verification passed. Client trusted-error promotion now refuses server messages containing raw online identifiers such as account/session/challenge/game/seek/report ids, with local and production verification passed. Account challenge directory/summary response shapes and account-history/head-to-head/shared public game-directory response shapes now fail closed on unsupported fields with local and production verification passed. Current local sub-slice: social profile/following/privacy response envelopes and privacy settings now fail closed on unsupported fields; the focused red/green regression, full online client/App/OnlineGameBrowser/HTTP server tests, `server:build`, `git diff --check`, app build, and local review have passed, but push, deploy, and production evidence are still outstanding. Continue the privacy pass with remaining response-shape checks before marking item 2 done.
