...
- Pick the first non-Done slice from `Next Ordered Slices`; items 1 through 10 are closed in the clean continuation worktree, and item 11 now has its approved design, the single-node runtime-coordinator foundation, PostgreSQL spectator-presence storage/wiring, the PostgreSQL runtime event outbox primitive, conservative runtime event publication wiring, conservative remote runtime-event polling, store-authoritative room refresh from remote snapshot hints for locally connected sockets, and missing-warm-room hydration for join/rejoin/account snapshot/spectate paths. The next item 11 sub-slice should implement another shared-runtime prerequisite from the approved design, preferably shared operation gates, drain behavior, startup maintenance ownership, rate-limit semantics, or two-instance tests, while still keeping `CASTLES_DEPLOYMENT_MODE=multi-instance` rejected until the full readiness gate is met.
