...
- Fold in the most useful Lichess-like friend shortcuts once the base graph is stable: pinned favorite friends and private notes are now local/account-scoped, account-game rows now expose follow/challenge shortcuts for registered opponents, People rows can jump to the visible Archive history for a player, and pending account challenges now surface a compact notice that jumps to the inbox; remaining work includes a richer game-end rematch prompt, challenge or rematch a followed player with the current setup, a true pair-scoped head-to-head history, and broader notification handling.
