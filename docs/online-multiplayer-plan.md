...
10. Phase 8 multi-instance/shared presence readiness.
   - Active next slice: audit and prepare the remaining single-node assumptions before any multi-instance deployment. Start with presence/spectator-count/shared-room assumptions and document or test the current single-node guardrails. Do not implement pub/sub, autoscaling, or public capacity claims unless this section is updated with a narrower design.

### Current Slice Selection Rule

- Pick the first non-Done slice from `Next Ordered Slices`; items 1 through 9 are closed in the clean continuation worktree, and the next active planned slice is item 10, multi-instance/shared presence readiness before public-scale traffic.
