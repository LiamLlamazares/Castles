...
11. Phase 8 multi-instance design and implementation.
   - Future slice: design and implement real multi-instance support only after a separate narrow design is approved. Required scope includes shared presence/pub-sub or equivalent WebSocket fanout, warm room cache invalidation or shared room reads for broadcasts, durable queue/lock coverage for every process-local gate, rolling WebSocket drain behavior, database pool/backpressure limits for multiple app instances, and multi-instance integration/load/rolling-deploy tests. Do not start this from the guardrail slice alone.

### Current Slice Selection Rule

- Pick the first non-Done slice from `Next Ordered Slices`; items 1 through 10 are closed in the clean continuation worktree, and the next active planned slice is item 11, real multi-instance design/implementation, unless Liam chooses to defer scale work and return to another lichess-like product slice first.
