...
- Fold in the most useful Lichess-like friend shortcuts once the base graph is stable: pinned favorite friends are now local/account-scoped; remaining work includes follow a recent opponent from the game end screen, challenge or rematch a followed player with the current setup, see followed players' public live games, jump to a friend-vs-friend head-to-head history, and show lightweight challenge notifications with accept/decline/cancel states.
