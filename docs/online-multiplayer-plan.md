...
- Fold in the most useful Lichess-like friend shortcuts once the base graph is stable: pinned favorite friends are now local/account-scoped, and account-game rows now expose follow/challenge shortcuts for registered opponents; remaining work includes a richer game-end rematch prompt, challenge or rematch a followed player with the current setup, see followed players' public live games, jump to a friend-vs-friend head-to-head history, and show lightweight challenge notifications with accept/decline/cancel states.
