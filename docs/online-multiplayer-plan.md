...
- Fold in the most useful Lichess-like friend shortcuts once the base graph is stable: pinned favorite friends and private notes are now local/account-scoped, followed/profile rows now expose current-setup challenge and copied direct-invite shortcuts, completed account-game rows now expose follow/challenge/current-setup rematch shortcuts for registered opponents and pass source-game metadata through the rematch callback, signed-in online game-end overlays now expose a rematch prompt when account history confirms a registered opponent, People rows can jump to the visible Archive history for a player, and pending account challenges now surface a compact notice that jumps to the inbox; remaining work includes friend-row rematch presets, a true pair-scoped head-to-head history, same-settings archive rematches once a full setup detail source exists, dedicated rematch request records, and broader notification handling.
