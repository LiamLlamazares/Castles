...
- Pick the first non-Done slice from `Next Ordered Slices`; items 1 through 10 are closed in the clean continuation worktree, and item 11 now has its approved design, the single-node runtime-coordinator foundation, and the first PostgreSQL spectator-presence storage primitive. The next item 11 sub-slice should either wire PostgreSQL spectator presence into the runtime coordinator/count path or implement another shared-runtime prerequisite from the approved design, such as PostgreSQL fanout/outbox, mandatory room hydration/invalidation, shared gates, drain behavior, or two-instance tests, while still keeping `CASTLES_DEPLOYMENT_MODE=multi-instance` rejected until the full readiness gate is met. Before using PostgreSQL spectator presence for shared counts, resolve the deferred clock-skew review item by choosing PostgreSQL-authoritative time or a documented/tested skew tolerance.
