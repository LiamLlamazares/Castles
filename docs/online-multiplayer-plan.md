...
2. Account/social privacy and moderation boundary pass.
   ...
   - Current item 2 notes: account challenge and account-history raw account id redaction is implemented with local and production verification passed. Client trusted-error promotion now refuses server messages containing raw online identifiers such as account/session/challenge/game/seek/report ids, with local and production verification passed. Account challenge directory/summary, account-history/head-to-head/shared public game-directory, social/profile/following/privacy, admin moderation report/audit, rating-leaderboard/public-rating, and account-report submission response shapes now fail closed on unsupported fields with local and production verification passed. The current social/privacy HTTP-boundary sub-slice makes store-returned profile, following-list profile, relationship mutation profile, and privacy-settings payloads fail closed before serialization when they contain unsupported internals such as raw account ids, relationship database keys, or token hashes; the focused red/green HTTP regression, full HTTP server tests, full PostgreSQL account-store tests, full online client tests, full App tests, full OnlineGameBrowser tests, `server:build`, `git diff --check`, app build, and local code/privacy review pass locally, with production deploy and evidence still outstanding. Continue the privacy pass with remaining response-shape checks before marking item 2 done.
...
- Pick the first non-Done slice from `Next Ordered Slices`; the next planned slice is item 2 unless a production blocker or user instruction overrides it.
