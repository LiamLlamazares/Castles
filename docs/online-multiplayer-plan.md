...
- Blocking must remove existing follow edges, prevent follows, challenges, messages if messages exist, followed-only lobby visibility, presence exposure, rematch suggestions, recent-opponent shortcuts, challenge inbox entries, and profile/action discovery where practical.
