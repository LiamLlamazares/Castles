...
- Pick the first non-Done slice from `Next Ordered Slices`; items 1 through 10 are closed in the clean continuation worktree, and item 11 now has its approved design plus the first single-node runtime-coordinator foundation sub-slice. The next item 11 sub-slice should implement one shared-runtime prerequisite from the approved design, such as PostgreSQL fanout/outbox, shared live spectator presence, mandatory room hydration/invalidation, shared gates, drain behavior, or two-instance tests, while still keeping `CASTLES_DEPLOYMENT_MODE=multi-instance` rejected until the full readiness gate is met.
