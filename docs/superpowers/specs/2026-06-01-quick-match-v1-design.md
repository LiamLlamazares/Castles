...
6. Server checks current public open seeks for a compatible seek not created by the same session.
...
- While pending, disable `Quick Match`, `Create Open Seek`, and row accept/cancel controls that would conflict.
...
- UI tests for setup-summary text matching the posted setup, keyboard activation, polite status announcements, focus restoration on failure, and keyboard reachability of the waiting owner panel.
